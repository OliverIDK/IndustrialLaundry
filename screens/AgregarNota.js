...
const AgregarNota = ({ navigation }) => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const tiposLavadoSnapshot = await getDocs(
          collection(database, "tipos_lavado")
        );
        const listaTiposLavado = tiposLavadoSnapshot.docs.map((doc) => ({
          id: doc.id,
          ...doc.data(),
        }));
  ...
  const continuar = () => {
  if (!clienteSeleccionado || !tipoLavadoSeleccionado || !tipoNotaSeleccionado) {
    alert("Por favor selecciona cliente, tipo de lavado y tipo de nota.");
    return;
  }
  ...
  return (
    <ScrollView style={styles.container}>
      ...
      <ScrollView
        ...
        contentContainerStyle={styles.lavadoScroll}
      >
